...
#model.get_first_stage_encoding = model.get_first_stage_encoding.to(device)
    ...
    if opt.bf16 == True:
        ...
        #model.model = ipex.optimize(model.model.eval(), dtype=torch.bfloat16, weights_prepack=False)
        #model.model = torch.compile(model.model, backend="ipex")
        
    if opt.ipex == True:
            model.model.diffusion_model.to(memory_format=torch.channels_last).eval()
            model.model.diffusion_model = ipex.optimize(model.model.diffusion_model.eval(), dtype= torch.bfloat16 if opt.bf16 else torch.float32, weights_prepack=True)
